...
def encode(
    ...
    for indices, samples in progress_batches(dataset, batch_size, "Diffusion encode"):
...
def decode(
    ...
    for indices, samples in progress_batches(dataset, batch_size, "Diffusion decode"):
...
def evaluate(
    ...
    for indices, samples in progress_batches(dataset, batch_size, "Diffusion evaluate"):
        targets = torch.stack([s["target"] for s in samples], dim=0).to(device)
        batch_shape = targets.shape
        cond = None
        if conditioning_mode in {"concatenate", "attention"}:
