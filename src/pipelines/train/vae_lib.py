...
def train(dataset, json_path: Path | str, val_dataset=None, resume: str | None = None) -> None:
    ...
    data_line = "Data: train_samples=%d%s | batch_size=%d | micro_batching=%s | num_workers=%d | epochs=%d | device=%s" % (
        len(dataset),
...
def train_via_new_api(dataset, json_path: Path | str, val_dataset=None, resume: str | None = None) -> None:
    ...
    trainer_cls = TRAINER_REGISTRY.get("vae")
    trainer = trainer_cls.from_config(json_path)
