...
def train(dataset, json_path: Path | str, val_dataset=None, resume: str | None = None) -> None:
    ...
    if "flow_matching" not in cfg:
        raise ValueError("Config does not declare a 'flow_matching' section.")

    training_cfg = cfg["training"]
    model_cfg = cfg["flow_matching"].get("unet", {})
    scheduler_cfg = cfg["flow_matching"].get("scheduler", {})
    ...
    conditioning_mode = resolve_conditioning_mode(
        training_cfg.get("conditioning") or cfg["flow_matching"].get("conditioning")
    )
    ...
    if conditioning_mode == "concatenate" and conditioning_eval is None:
        logging.warning("Flow matching config requested LDCT conditioning but dataset samples did not expose 'image'.")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if save_samples:
            ...
            grid = utils.make_grid(vis, rows, cols)
            utils.save_image(grid, output_dir / "samples" / f"epoch{epoch:04d}.png")
            model.train()
